...
const logIn = (user, method) => dispatch =>
  axios
    ...
    .catch(err =>
      dispatch(callActions([err.response.status, err.response.data]))
    );

export const forgotPW = email => dispatch =>
  axios
    .post('/api/auth/forgotPW', email)
    .then(email =>
      dispatch(
        callActions([
          email.status,
          `Please check your inbox at ${
            email.data
          } for email from ForAnimals. Remember to change your password within 24 hours!`
        ])
      )
    )
    .catch(err =>
  ...
  axios
    ...
    .then(email =>
      dispatch(
        callActions([
          email.status,
          `Congratulations, you have created an account with For Animals under ${
            email.data
          }. Watch for an email from us that gives you access!`
        ])
      )
    )
    .catch(err =>
...
export const logInAndWelcome = user => dispatch =>
  dispatch(logIn(user, 'logIn')).then(user =>
    user ? history.push('/welcome') : null
  ); //only redirect if user return by logIn
