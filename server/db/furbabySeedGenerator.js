...
const defaultJSON = () => {
  ...
  const intakeDate = moment(
    Math.min(moment(birthDate).add(addlIntakeDay, 'days'), moment())
  ).format('MM-DD-YYYY');
  const addlAdoptionDate = chance.integer({ min: addlIntakeDay, max: 1825 });
  const adoptionDate = moment(birthDate)
    .add(addlIntakeDay, 'days')
    .add(addlAdoptionDate, 'days')
    .format('MM-DD-YYYY');
  return {
    ...
    currentStatus:
      currentStatusArray[
        chance.integer({ min: 0, max: currentStatusArray.length - 1 })
      ],
    ...
    goodWithCats:
      goodVars[chance.integer({ min: 0, max: goodVars.length - 1 })],
    goodWithDogs:
      goodVars[chance.integer({ min: 0, max: goodVars.length - 1 })],
    goodWithChildren:
      goodVars[chance.integer({ min: 0, max: goodVars.length - 1 })],
    specialNeeds: capitalize(chance.sentence({ words: 10 })),
    ...
    photoUrl: {
      downloadURL: imageURLArr[chance.integer({ min: 0, max: 199 })],
      path: imageURLArr[chance.integer({ min: 0, max: 199 })]
    },
...
const createFurbabySeed = function() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (adoptionStatusArray.indexOf(currentStatus) >= 0)
      seedObj.parentId = null;
    if (seedObj.parentId === null) seedObj.adoptionDate = null;
    seedObj.adoptedName =
      seedObj.parentId === null ? '' : chance.first({ nationality: 'it' });
    seed.push(seedObj);
  }
  return seed;
};
