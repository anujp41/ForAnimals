...
router.post('/logIn', function(req, res, next) {
  passport.authenticate('local-login', { failureFlash: true }, function(
    err,
    user,
    info
  ) {
      ...
      if (user === null) {
        User.create(req.body)
          ...
          .then(user => {
            ...
            sendAccessEmail(id, email, firstName, lastName).then(email =>
              res.json(email)
            );
          });
      } else {
        ...
        if (googleId) {
          req.flash(
            'google-signUp',
            `${email} was previously used to sign up via Google. Please log in with Google again!`
          );
          ...
          req.flash(
            'email-exists',
            `There is an existing account under ${email}. Please login or change your password if forgotten!`
          );
  ...
  User.findOne({ where: { email } })
    ...
    .then(resEmail => {
      if (resEmail === null) {
        req.flash(
          'email-not-found',
          `Cannot locate ${email}. Please try entering email again!`
        );
        ...
        req.flash(
          'google-login',
          `You have signed in with Google using ${
            resEmail.email
          }. Please try singing in with google again!`
        );
        ...
        sendPWEmail(email, firstName)
          .then(emailSent => {
            ...
            ResetPWLog.create(emailSent).then(() => {
              res.status(200);
  ...
  User.update(
    ...
    .then(() => {
      if (access) {
        ...
        return res.json(
          `${firstName} ${lastName} (${email}) was successfully granted access. Thank you for your prompt response!`
        );
      } else if (!access) {
        sendDenyEmail(email, firstName);
        return res.json(
          `${firstName} ${lastName} (${email}) was successfully denied access. Thank you for your prompt response!`
        );
      }
    })
    .catch(next);
